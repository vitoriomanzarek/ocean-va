/**
 * Update VAs with Profile Slug (Link type) and Video URL (Video Link type)
 * Run with: node scripts/updateVAsProfileSlugAndVideo.js
 */
...
async function main() {
  try {
    console.log('🔗 Updating VAs with Profile Slug (Link) and Video URL (Video Link)...\n');
    ...
    for (const va of vasData) {
      ...
      try {
        const fieldData = {
          'profile-slug': va.slug,  // Link type
          'video-url': va.videoUrl || null,  // Video Link type
        };
