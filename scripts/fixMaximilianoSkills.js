/**
 * Script para corregir el campo skills-richtext de Maximiliano
 * Quita el contenedor <div class="va-skills-container"> que causa doble fondo
 */
...
function fixSkillsHTML(html) {
  ...
  if (hasSkillTags) {
    ...
    return html.replace(/<div\s+class=["']va-skills-container["'][^>]*>/g, '').replace(/<\/div>\s*$/, '');
  }
...
async function main() {
  console.log('🔧 Corrigiendo skills-richtext de Maximiliano...\n');
  ...
  const currentSkills = maximiliano.fieldData['skills-richtext'] || '';
  console.log(`Campo: skills-richtext`);
  console.log(`Longitud: ${currentSkills.length} caracteres`);
  console.log(`Contiene contenedor: ${currentSkills.includes('va-skills-container') ? '✅ SÍ' : '❌ NO'}`);
  ...
  if (fixedSkills === currentSkills) {
    console.log('⚠️  No se encontró contenedor para remover. El campo ya está correcto o tiene un formato diferente.');
    console.log('   Revisando si necesita corrección...\n');
    
    // Verificar si tiene el contenedor pero con otro formato
    if (currentSkills.includes('va-skills-container')) {
      console.log('   ⚠️  Se detectó "va-skills-container" pero no se pudo extraer correctamente.');
      ...
      if (altFixed !== currentSkills) {
        ...
        await updateField(apiClient, vaCollection.id, maximiliano.id, altFixed, currentSkills);
        return;
      }
    }
  ...
  await updateField(apiClient, vaCollection.id, maximiliano.id, fixedSkills, currentSkills);
}
