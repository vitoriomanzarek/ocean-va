/**
 * Create missing specializations for Maximiliano in Webflow CMS
 * Run with: node scripts/createMaximilianoSpecializations.js
 */
...
// 4 specializations for Maximiliano's cards
...
async function main() {
  try {
    console.log('🔗 Creating missing specializations for Maximiliano...\n');
    ...
    // Now assign them to Maximiliano
    console.log('═'.repeat(80));
    console.log('\n🔗 Assigning specializations to Maximiliano...\n');
    ...
    // Find Maximiliano
    console.log('📥 Fetching Maximiliano from Webflow...');
    const vasResponse = await client.getCollectionItems(vaCollection.id, { limit: 200 });
    const maximiliano = vasResponse.items.find(
      (va) => va.fieldData.name === 'Maximiliano' || va.fieldData.name === 'MAXIMILIANO'
    );

    if (!maximiliano) {
      console.error('❌ Maximiliano not found in Webflow CMS');
      console.log('💡 Make sure Maximiliano has been added to Webflow first.\n');
      process.exit(1);
    }

    console.log(`✅ Found Maximiliano (ID: ${maximiliano.id})\n`);

    // Get all 4 card specializations (including Commercial Insurance which already exists)
    ...
    // Update Maximiliano with specializations
    const fieldData = {
      'specialization': specializationIds,
    };

    console.log('🔄 Updating Maximiliano with specializations...\n');

    await client.updateCollectionItem(vaCollection.id, maximiliano.id, fieldData, {
      isDraft: false,
    });

    console.log(`✅ Maximiliano updated successfully with ${specializationIds.length} specializations!`);
    console.log(`   Item ID: ${maximiliano.id}`);
    console.log(`   Specializations: ${cardSpecializations.join(', ')}\n`);
